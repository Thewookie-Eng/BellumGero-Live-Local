...
public:
	...
	static void show(CreatureObject* player) {
		...
		if (!manager->getSelectedTravelPoint(player, point, exists, message)) {
			player->sendSystemMessage("[World Builder] ERROR: " + message);
			return;
		}
		...
		if (!exists) {
			status << "No Travel Point. Create one by naming it.";
		} else {
			status << point.pointName << "\nArrival x/z/y: " << point.x << " / " << point.z << " / " << point.y
		...
		box->setPromptText(status.toString());
		box->setUsingObject(player);
		box->setCancelButton(true, "@cancel");
		box->setOkButton(true, "@ok");
		box->setCallback(new WorldBuilderTravelPointListCallback(player->getZoneServer()));
		box->addMenuItem("Status", STATUS);
		box->addMenuItem("Create / Rename Travel Point", RENAME);
		box->addMenuItem("Set Arrival Point to My Current Position", ARRIVAL);
		box->addMenuItem("Toggle Incoming Travel", INCOMING);
		box->addMenuItem("Toggle Interplanetary Travel", INTERPLANETARY);
		box->addMenuItem("Set Landing Range", RANGE);
		box->addMenuItem("Remove Travel Point", REMOVE);
		box->addMenuItem("Back", BACK);
...
inline void WorldBuilderTravelPointInputCallback::run(CreatureObject* player, SuiBox*, uint32 eventIndex, Vector<UnicodeString>* args) {
	...
	try {
		if (action == WorldBuilderTravelPointEditor::RENAME) {
			ok = manager->setSelectedTravelPointName(player, value, message);
		} else if (action == WorldBuilderTravelPointEditor::RANGE) {
			ok = manager->setSelectedTravelPointLandingRange(player, Float::valueOf(value), message);
		}
	} catch (Exception&) {
		message = "Invalid value.";
	}

	player->sendSystemMessage(String("[World Builder] ") + (ok ? "" : "ERROR: ") + message);
...
inline void WorldBuilderTravelPointListCallback::run(CreatureObject* player, SuiBox*, uint32 eventIndex, Vector<UnicodeString>* args) {
	...
	if (action == WorldBuilderTravelPointEditor::RANGE) {
		WorldBuilderTravelPointEditor::input(player, action, "Landing Range", "Enter 0.5 through 64 meters:");
		return;
	}

	if (action == WorldBuilderTravelPointEditor::ARRIVAL) {
		ok = manager->setSelectedTravelPointArrival(player, message);
	} else if (action == WorldBuilderTravelPointEditor::INCOMING) {
		ok = manager->toggleSelectedTravelPointIncoming(player, message);
	} else if (action == WorldBuilderTravelPointEditor::INTERPLANETARY) {
		ok = manager->toggleSelectedTravelPointInterplanetary(player, message);
	} else if (action == WorldBuilderTravelPointEditor::REMOVE) {
		ok = manager->removeSelectedTravelPoint(player, message);
	} else if (action == WorldBuilderTravelPointEditor::BACK) {
		return;
	} else {
		ok = true;

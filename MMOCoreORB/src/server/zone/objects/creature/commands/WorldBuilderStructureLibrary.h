/*
 * WorldBuilderStructureLibrary.h
 *
...
#ifndef WORLDBUILDERSTRUCTURELIBRARY_H_
#define WORLDBUILDERSTRUCTURELIBRARY_H_
...
class WorldBuilderStructureLibraryRootSuiCallback;
class WorldBuilderStructureLibraryListSuiCallback;
class WorldBuilderStructureLibrarySearchSuiCallback;
class WorldBuilderStructureLibraryActionSuiCallback;

class WorldBuilderStructureLibrary {
private:
	...
	enum CompatibilityResult {
		...
		COMPATIBLE_STRUCTURE = 1,
		INCOMPATIBLE_STRUCTURE = 2,
	...
	static CompatibilityResult inspectCompatibleTemplate(const std::string& path, int& cellCount) {
		...
		String portalLayoutFile = buildingTemplate->getPortalLayoutFilename();
		if (portalLayoutFile.isEmpty() || portalLayoutFile.length() <= 1)
			return INCOMPATIBLE_STRUCTURE;

		// TreeArchive::getBytes() logs a warning when a record is absent. Check the
		// directory index first so incomplete/unused registered templates can be
		// excluded without polluting the server console.
		if (!archiveContainsFile(portalLayoutFile))
			return MISSING_PORTAL_ASSET;
		...
		cellCount = portalLayout->getCellTotalNumber();
	...
	static std::string displayNameFromPath(const std::string& path, int cellCount, bool includeCategory) {
		...
		label += "  | " + std::to_string(cellCount) + " cells";
		return label;
	}
	...
	static LibraryCatalog buildCatalog() {
		...
		while (iterator.hasNext()) {
			...
			if (result == INCOMPATIBLE_STRUCTURE) {
				++catalog.skippedIncompatibleStructures;
				continue;
			}
			if (result != COMPATIBLE_STRUCTURE)
				continue;
		...
		std::sort(catalog.entries.begin(), catalog.entries.end(), [](const StructureEntry& a, const StructureEntry& b) {
			if (a.label == b.label)
	...
	static const StructureEntry* findCatalogEntry(const String& templatePath) {
		std::string wanted(templatePath.toCharArray());
		const std::vector<StructureEntry>& entries = getCatalog().entries;
	...
	static void collectMatches(const String& category, const String& query,
		...
		for (size_t i = 0; i < entries.size(); ++i) {
			const StructureEntry& entry = entries[i];
	...
	static int getCellCount(const String& templatePath) {
		const StructureEntry* entry = findCatalogEntry(templatePath);
...
class WorldBuilderStructureLibraryRootSuiCallback : public SuiCallback {
	Vector<String> actions;

public:
	WorldBuilderStructureLibraryRootSuiCallback(ZoneServer* server, const Vector<String>& menuActions)
	...
	void run(CreatureObject* player, SuiBox* suiBox, uint32 eventIndex, Vector<UnicodeString>* args) {
		...
		if (action == "@search") {
			WorldBuilderStructureLibrary::showSearchInput(player);
		} else if (action == "@all") {
			WorldBuilderStructureLibrary::showTemplateList(player, "all", "", 0);
		} else if (action == "@back") {
			WorldBuilderCommandUi::showMenu(player);
		} else {
			WorldBuilderStructureLibrary::showTemplateList(player, action, "", 0);
		}
	}
};

class WorldBuilderStructureLibraryListSuiCallback : public SuiCallback {
	String category;
	String query;
	int page;
	Vector<String> actions;
...
public:
	WorldBuilderStructureLibraryListSuiCallback(ZoneServer* server, const String& categoryName,
	...
	void run(CreatureObject* player, SuiBox* suiBox, uint32 eventIndex, Vector<UnicodeString>* args) {
		...
		if (action == "@prev") {
			WorldBuilderStructureLibrary::showTemplateList(player, category, query, page - 1);
			return;
		}
		if (action == "@next") {
			WorldBuilderStructureLibrary::showTemplateList(player, category, query, page + 1);
		...
		WorldBuilderStructureLibrary::showStructureActions(player, action, category, query, page);
	}
};

class WorldBuilderStructureLibrarySearchSuiCallback : public SuiCallback {
public:
	WorldBuilderStructureLibrarySearchSuiCallback(ZoneServer* server) : SuiCallback(server) {
	}
	...
	void run(CreatureObject* player, SuiBox* suiBox, uint32 eventIndex, Vector<UnicodeString>* args) {
		...
		WorldBuilderStructureLibrary::showTemplateList(player, "all", query, 0);
	}
};

class WorldBuilderStructureLibraryActionSuiCallback : public SuiCallback {
	String templatePath;
	String category;
	String query;
	int page;
...
public:
	WorldBuilderStructureLibraryActionSuiCallback(ZoneServer* server, const String& selectedTemplate,
	...
	void run(CreatureObject* player, SuiBox* suiBox, uint32 eventIndex, Vector<UnicodeString>* args) {
		...
		if (eventIndex == 1) {
			WorldBuilderStructureLibrary::showTemplateList(player, category, query, page);
			return;
		}
		...
		if (action == "@preview") {
			result = WorldBuilderStructurePreview::spawn(player, templatePath, 15.0f, message);
			player->sendSystemMessage(String("[World Builder] ") + (result ? "" : "ERROR: ") + message);
			WorldBuilderStructureLibrary::showStructureActions(player, templatePath, category, query, page);
			return;
		}
		...
		if (action == "@add") {
			...
			if (player->getParentID() == 0) {
				String ignored;
				WorldBuilderStructurePreview::clear(player, ignored);
			}

			result = WorldBuilderManager::instance()->addStructure(player, templatePath, 15.0f, message);
			player->sendSystemMessage(String("[World Builder] ") + (result ? "" : "ERROR: ") + message);
			if (result)
				WorldBuilderCommandUi::showMenu(player);
			else
...
inline void WorldBuilderStructureLibrary::showRoot(CreatureObject* player) {
	...
	if (manager == nullptr || !manager->hasSession(player)) {
		WorldBuilderCommandUi::sendMessage(player, "World Builder Structure Library",
			"Open or create a World Builder project before browsing structures.");
		return;
	}

	const LibraryCatalog& catalog = getCatalog();
	const std::vector<StructureEntry>& entries = catalog.entries;
	...
	box->setPromptTitle("World Builder Structure Library");
	...
	box->setCallback(new WorldBuilderStructureLibraryRootSuiCallback(player->getZoneServer(), actions));
	player->getPlayerObject()->addSuiBox(box);
	player->sendMessage(box->generateMessage());
}

inline void WorldBuilderStructureLibrary::showSearchInput(CreatureObject* player) {
	if (player == nullptr || player->getPlayerObject() == nullptr)
		return;

	ManagedReference<SuiInputBox*> box = new SuiInputBox(player, SuiWindowType::NONE);
	box->setPromptTitle("Search World Builder Structure Library");
	...
	box->setUsingObject(player);
	box->setCallback(new WorldBuilderStructureLibrarySearchSuiCallback(player->getZoneServer()));
	box->setForceCloseDistance(-1);
	player->getPlayerObject()->addSuiBox(box);
	player->sendMessage(box->generateMessage());
}

inline void WorldBuilderStructureLibrary::showTemplateList(CreatureObject* player, const String& category, const String& query, int page) {
	if (player == nullptr || player->getPlayerObject() == nullptr)
		return;
	...
	if (total == 0) {
		...
		WorldBuilderCommandUi::sendMessage(player, "World Builder Structure Library", text.toString());
		return;
	}
	...
	box->setPromptTitle(query.isEmpty() ? "Structure Library - " + categoryLabel : "Structure Library Search");
	...
	box->setCallback(new WorldBuilderStructureLibraryListSuiCallback(player->getZoneServer(), category, query, page, actions));
	player->getPlayerObject()->addSuiBox(box);
	player->sendMessage(box->generateMessage());
}

inline void WorldBuilderStructureLibrary::showStructureActions(CreatureObject* player, const String& templatePath,
	const String& category, const String& query, int page) {
	if (player == nullptr || player->getPlayerObject() == nullptr)
		return;
	...
	ManagedReference<SuiListBox*> box = new SuiListBox(player, SuiWindowType::NONE, SuiListBox::HANDLESINGLEBUTTON);
	box->setPromptTitle("Structure: " + displayNameForTemplate(templatePath));

	StringBuffer prompt;
	prompt << "SERVER template:\n" << templatePath
		   << "\n\nInterior cells: " << cellCount
	...
	Vector<String> actions;
	box->addMenuItem("Preview Structure (Transient, 15m Ahead)", actions.size() + 1);
	actions.add("@preview");
	box->addMenuItem("Clear Current Structure Preview", actions.size() + 1);
	actions.add("@clear");
	box->addMenuItem("Add Structure to Active Project (15m Ahead)", actions.size() + 1);
	actions.add("@add");
	box->addMenuItem("Back to Structure List", actions.size() + 1);
	actions.add("@back");

	box->setCallback(new WorldBuilderStructureLibraryActionSuiCallback(player->getZoneServer(), templatePath,
		category, query, page, actions));
	player->getPlayerObject()->addSuiBox(box);
	player->sendMessage(box->generateMessage());
}

#endif /* WORLDBUILDERSTRUCTURELIBRARY_H_ */
